...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>thanhdeptrai chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "thanhdeptrai", HidePremium = false, IntroText = "thanhdeptrai⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("thanhdeptrai SCRIPT")
W:AddLabel("Discordthanhdeptrai")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "thanhdeptrai(Noitification)",
